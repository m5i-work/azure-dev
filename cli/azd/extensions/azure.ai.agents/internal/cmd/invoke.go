...
the current turn. For remote Invocations agents, message-free --resume performs one
best-effort GET of the latest saved invocation without polling or interpreting its
lifecycle. Use --cancel to cancel saved Responses work. In multi-agent projects, use
--agent-name to select the saved work for --resume or --cancel. Saved-work operations
...
func (a *InvokeAction) Run(ctx context.Context) error {
	...
	if (a.flags.resumable || a.flags.resume || a.flags.cancel) && protocol != agent_api.AgentProtocolResponses {
		...
		invocationsGet := protocol == agent_api.AgentProtocolInvocations && a.flags.resume &&
			!a.flags.resumable && !a.flags.cancel
		if !invocationsGet {
			return exterrors.Validation(
				exterrors.CodeInvalidParameter,
